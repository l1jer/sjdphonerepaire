...
function logFormData(data: RepairFormData, caseId: string): boolean {
  try {
    ...
    console.log(`Brand: ${data.deviceBrand}`)
    console.log(`Model: ${data.deviceModel}`)
    console.log(`Serial: ${data.serialNumber}`)
    console.log(`IMEI: ${data.imeiNumber}`)
    console.log(`Condition: ${data.deviceCondition}`)
    console.log('\n=== END SUBMISSION ===\n')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const photoPromises = devicePhotos.map(async (photo, index) => {
      console.log(`Processing photo ${index + 1}: ${photo.name} (${photo.size} bytes)`)
      return {
        name: photo.name,
        size: photo.size,
        type: photo.type,
    ...
    console.log(`PDF generation completed in ${pdfGenerationTime}ms. Size: ${pdfBuffer.length} bytes`)
